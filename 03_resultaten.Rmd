...
De getijstuw in Dendermonde bevindt zich op de Dender, net stroomopwaarts van de samenvloeiing met de Zeeschelde (Figs. \@ref(fig:fotostuw1) - \@ref(fig:fotostuw4)).
De stuw ligt ter hoogte van het getijdengebonden deel van de Schelde en ligt binnen een overstromingsgevoelige zone met een uitgesproken estuarien karakter.
...
De stuw fungeert als overgangsstructuur tussen het estuarien beïnvloede getijregime en het meer gestabiliseerde waterpeil in de bovenlopen van het riviersysteem.
...
Sinds eind 2024 is de werking van de stuw volledig geautomatiseerd en dit brengt een aantal wijzigingen met zich mee.
Zo zou nu de onderste schuif meer geopend worden dan vroeger (communicatie met DVW).

```{r fotostuw1, fig.cap = "Water stroomt bij het einde van vloed over de stuw van de Zeeschelde naar de Dender op 20 februari 2025 om 9:48 (lokale tijd).", fig.show = "hold", out.width = "50%", fig.align = "center"}
include_graphics(path = "./media/pictures/20250220/20250220_094818.jpg")
```

```{r fotostuw2, fig.cap = "Zicht op de stuwgeul in stroomafwaartse richting. De samenvloeiing van de Dender met de Zeeschelde is zichtbaar op 20 februari 2025 om 9:48 (lokale tijd).", fig.show = "hold", out.width = "50%", fig.align = "center"}
include_graphics(path = "./media/pictures/20250220/20250220_094844.jpg")
```

```{r fotostuw3, fig.cap = "Zicht op de sluisgeul (links) en de stuwgeul (rechts) in stroomopwaartse richting op 20 februari 2025 om 9:49 (lokale tijd).", fig.show = "hold", out.width = "50%", fig.align = "center"}
include_graphics(path = "./media/pictures/20250220/20250220_094859.jpg")
```

```{r fotosluis1, fig.cap = "Zicht op de sluisgeul in stroomafwaartse richting op 20 februari 2025 om 9:48 (lokale tijd).", fig.show = "hold", out.width = "50%", fig.align = "center"}
include_graphics(path = "./media/pictures/20250220/20250220_094852.jpg")
```

```{r fotostuw4, fig.cap = "Water stroomt bij het begin van vloed over de stuw van de Dender naar de Zeeschelde op 8 september 2025 om 13:40 (lokale tijd).", fig.show = "hold", out.width = "50%", fig.align = "center"}
include_graphics(path = "./media/pictures/20250908/20250908_134018.jpg")
```
...
Aan de linkerzijde bevinden zich de doorstroomopeningen van de stuw, geflankeerd door oeverconstructies en geleidewerken.
Deze stuwsecties laten gecontroleerde waterbeweging toe in functie van het getij en het bovenstroomse peilbeheer.
De schutsluis is parallel aan de stuw gelegen langs rechts en dient voor de doorgang van de scheepvaart.
Ze verbindt het benedenstroomse getijdeel van de Schelde met het hoger gelegen niet-getijgebonden deel van de Dender.

De dwarsdoorsneden van het complex tonen telkens **links de stuw** en **rechts de schutsluis (**Fig.\@ref(fig:doorsnedeoverzicht), \@ref(fig:doorsnedebc), \@ref(fig:doorsnedede) en \@ref(fig:doorsnedef)**)**.

-   **Links** bevindt zich de stuw, opgebouwd uit betonnen funderingen en verticale schuiven die in hoogte verstelbaar zijn.
    Deze schuiven regelen de doorstroming en stabiliseren het bovenstroomse peil rond **3,82 m TAW**.

-   **Rechts** is de sluis zichtbaar, met de kolk en sluishoofden.
    Hier worden schepen geschut tussen het hoger gelegen bovenstroomse peil en het variabele benedenstroomse peil dat onder invloed staat van het getij.

De doorsneden tonen ook de verhouding tussen de breedte van de stuw en die van de sluis, en illustreren hoe beide onderdelen parallel in het rivierprofiel zijn ingeplant.
De meer benedenstroomse doorsnede F geeft daarbij extra inzicht in de uitstromingszone: hier mondt het water vanuit stuw en sluis uit in de Schelde.

De langsdoorsnede A (Fig. \@ref(fig:doorsnedea)) geeft een detailbeeld van de schutsluis zelf.
In dit profiel zijn de kolk, de sluishoofden en de deuren zichtbaar.
De tekening maakt het verschil in waterpeil duidelijk tussen de bovenstroomse zijde (opgestuwd rond 3,82 m TAW) en de benedenstroomse zijde (onderhevig aan het getij).
Het schutproces gebeurt via inlaten in de sluishoofden, zodat het kolkpeil gecontroleerd kan worden aangepast aan boven- of benedenstroomse omstandigheden.

Het sluis- en stuwcomplex van Dendermonde is een geïntegreerde waterbouwkundige structuur waarbij stuw en sluis parallel in de rivier zijn opgesteld.
In de doorsneden komt dit duidelijk naar voren: links de stuw die het bovenstroomse peil reguleert, rechts de sluis die de scheepvaart doorgang verleent.

```{r doorsnedeoverzicht, fig.cap = "Bovenaanzicht", fig.show = "hold", out.width = "100%", fig.align = "default"}

# Define the input and output file paths
input_file <- "./media/plans/B36043.tif"
output_file <- "./media/plans/B36043.png"

# Read the image
img <- image_read(input_file)

# Write the image to a new file with the .png extension
image_write(img, path = output_file, format = "png")
include_graphics(path = "./media/plans/B36043.png")
```

```{r doorsnedea, fig.cap = "Langsdoorsnede A", fig.show = "hold", out.width = "100%", fig.align = "default"}

# Define the input and output file paths
input_file <- "./media/plans/doorsnedeA.tif"
output_file <- "./media/plans/doorsnedeA.png"
...
# Write the image to a new file with the .png extension
image_write(img, path = output_file, format = "png")
include_graphics(path = "./media/plans/doorsnedeA.png")
```

```{r doorsnedebc, fig.cap = "Dwarsdoorsnede B-C", fig.show = "hold", out.width = "100%", fig.align = "default"}

# Define the input and output file paths
input_file <- "./media/plans/doorsnedeBC.tif"
output_file <- "./media/plans/doorsnedeBC.png"
...
# Write the image to a new file with the .png extension
image_write(img, path = output_file, format = "png")
include_graphics(path = "./media/plans/doorsnedeBC.png")
```

```{r doorsnedede, fig.cap = "Dwarsdoorsnede D-E", fig.show = "hold", out.width = "100%", fig.align = "default"}

# Define the input and output file paths
input_file <- "./media/plans/doorsnedeDE.tif"
output_file <- "./media/plans/doorsnedeDE.png"
...
# Write the image to a new file with the .png extension
image_write(img, path = output_file, format = "png")
include_graphics(path = "./media/plans/doorsnedeDE.png")
```

```{r doorsnedef, fig.cap = "Dwarsdoorsnede F", fig.show = "hold", out.width = "100%", fig.align = "default"}

# Define the input and output file paths
input_file <- "./media/plans/doorsnedeF.tif"
output_file <- "./media/plans/doorsnedeF.png"
...
# Write the image to a new file with the .png extension
image_write(img, path = output_file, format = "png")
include_graphics(path = "./media/plans/doorsnedeF.png")
```
...
-   **Terreinbezoeken en overlegmomenten**
    -   Bezoek aan de getijdestuw op de Dijle te Mechelen op 1 september 2025, waar sinds 2010 een aangepaste ‘vismigratiestand’ gehanteerd wordt [@stevens2010]. De getijdestuw was twee jaar buiten werking en werd terug in gebruik genomen midden augustus 2025.
    -   Bezoek aan de getijdestuw op de Dender te Dendermonde op 20 februari 2025.
...
```{r tijdreeksenstudieperiode1, fig.height=12, fig.width=10, fig.cap="Overzicht van metingen van waterpeil (m TAW), voor relevante meetstations doorheen de tijd van 25 tot 28 januari 2024. De stations staan geordend van stroomopwaarts naar stroomafwaarts. Noteer hoe getracht wordt om het stroomopwaarts peil niet onder de 3.82 m TAW te laten komen."}
...
```{r tijdreeksenstudieperiode2, fig.height=12, fig.width=10, fig.cap="Overzicht van metingen van waterpeil (m TAW), voor relevante meetstations doorheen de tijd van 25 tot 28 januari 2024. De stations staan geordend van stroomopwaarts naar stroomafwaarts. Noteer hoe getracht wordt om het stroomopwaarts peil niet onder de 3.82 m TAW te laten komen."}
...
In het geval van stroming over de stuw gaat en stroomopwaarts en stroomafwaarts peil hoger zijn dan de bovenkant van de stuw is er nog steeds enige hinder van de schuif die dan volledig onder water staat.
In dat geval spreken we hier echter nog steeds van "ongehinderde" stroming maar gebruiken we "" om deze kanttekening mee te nemen.
...
\- $Q_o$ = Discharge over the weir/ Debiet over de stuw (m³/s)

\- $C_e$ = Discharge coefficient/ Debietscoefficiënt (dimensionless/ dimensieloos)

\- $b$ = Width of the weir/ Breedte van de stuw (m)

\- $g$ = Gravitational acceleration/ Zwaartekrachtversnelling (9.81 m/s²)

\- $h_o$ = Head over the weir crest/ Waterhoogte boven de stuwkruin (m)
...
\- $Q_u$ = Discharge under the weir/ Debiet onder de stuw (m³/s)

\- $C_d$ = Discharge coefficient for underflow/ Debietscoefficiënt voor onderstroom (dimensionless/ dimensieloos)

\- $h_u$ = Height of weir opening/ Hoogte stuw opening (m)

\- $h_{up}$ = Upstream water level/ Stroomopwaarts waterpeil (m)

\- $h_{down}$ = Downstream water level/ Stroomafwaarts waterpeil (m)
...
## Stuwwerking in relatie tot de bovenafvoer en het getijdenregime

Stroomafwaarts van de getijsluis wordt een uitgesproken getijdenregime waargenomen (Fig. \@ref(fig:tijdsreeksberekeningen)) met laag- en hoogwaterstanden tussen de 2.86 m (0.91 m TAW) en 8.46 m (6.41 m TAW).
...
ggplot(plot_data, aes(x = date_utc, y = h_downstream)) +
  ...
  # ) +
  labs(
    title = "Stroomafwaarts waterpeil voor meerdere getijcycli",
    x = "Datum",
...
as.data.frame(table(data$tide_direction, data$date)) %>%
  pivot_wider(
    names_from = Var1, 
    values_from = Freq) %>%
  mutate(ratio=Eb/Vloed) %>%
  summarize(median=median(ratio))
```

## Stuwwerking in relatie tot bereikbaarheid en passeerbaarheid
...
Om vispassage te evalueren werden twee zaken in beschouwing genomen: (1) Contact tussen het stroomopwaarts en stroomafwaarts pand via water en (2) stroomsnelheid.
...
ggplot(data %>% dplyr::filter(V_o < 8), aes(x = date_utc)) +
  ...
  scale_color_manual(
    values = c("no" = "red", "yes" = "green"), # De kleuren zijn nog steeds gekoppeld aan de oorspronkelijke data-waarden ("no", "yes")
    labels = c("no" = "nee", "yes" = "ja"),    # De labels voor de legende worden hier ingesteld
    name = "Bereikbaar over stuw?"              # Optioneel: Herbevestig de legende titel
  ) + 
  theme_minimal(base_size = 17) +
  theme(
...
```{r puntenover, fig.width=12, fig.height=8, out.width='100%', fig.align='center', fig.cap="Tijdreeksen van waterstroomsnelheid over de stuw met onderscheid tussen passeerbaar (groen) en niet-passeerbaar (rood)."}
ggplot(data %>% dplyr::filter(V_o < 6), aes(x = date_utc)) +
  ...
  scale_color_manual(
    values = c("no" = "red", "yes" = "green"), # Koppeling van data-waarden ("no", "yes") aan kleuren
    labels = c("no" = "nee", "yes" = "ja"),    # Vervangt de legende tekst voor deze waarden
    name = "Passeerbaar over stuw?"             # Zorgt ervoor dat de legende titel behouden blijft
  ) + 
  theme_minimal(base_size = 17) +
  theme(
    legend.position = "bottom"  # move legend below
...
```{r puntenonder, fig.width=12, fig.height=8, out.width='100%', fig.align='center', fig.cap="Tijdreeksen van waterstroomsnelheid onder de stuw met onderscheid tussen passeerbaar (groen) en niet-passeerbaar (rood)."}
ggplot(data %>% dplyr::filter(V_o < 6), aes(x = date_utc)) +
  ...
  scale_color_manual(
    values = c("no" = "red", "yes" = "green"), # Bepaalt de kleuren gebaseerd op de data-waarden
    labels = c("no" = "nee", "yes" = "ja"),    # Past de tekst in de legende aan
    name = "Passeerbaar onder stuw?"            # Optioneel: Herbevestigt de legende titel
  ) + 
  theme_minimal(base_size = 17) +
  theme(
    legend.position = "bottom"
...
Water stroomt vooral in stroomafwaarts richting wat logisch is aangezien er naast het eb-gedreven getij ook een belangrijk aandeel bovenafvoer is dat moet weggewerkt worden.

Gedurende eb wordt er voornamelijk water stroomafwaarts afgevoerd (89.5 % van de tijd) over (65.3 % van de tijd) en onder de stuw (28.9 % van de tijd) (Tabel \@ref(tab:ebvloed)).
Gedurende eb is er geen waterstroming gedurende 9.4 % van de tijd.
Gedurende eb wordt water stroomopwaarts afgevoerd voor slechts 1.1 % van de tijd.
Daarbij gaat het water respectievelijk 0.8 % en 0.3 % van de tijd over en onder de stuw in stroomopwaartse richting.

Gedurende vloed wordt er voornamelijk water stroomafwaarts afgevoerd (62.7 % van de tijd) over (48.0 % van de tijd) en onder de stuw (18.2 % van de tijd).
Gedurende vloed is er geen waterstroming gedurende 26.8 % van de tijd.
Gedurende vloed wordt water stroomopwaarts afgevoerd voor 11.3 % van de tijd.
Daarbij gaat het water respectievelijk 6.7 % en 5.4 % van de tijd over en onder de stuw in stroomopwaartse richting.
...
data %>%
  ...
  summarise(
    ...
    # Remaining metrics
    no_flow = sum(Q_o == 0 & Q_u == 0, na.rm = TRUE),
    both_flow_pos = sum(Q_o > 0 & Q_u > 0, na.rm = TRUE),
    both_flow_neg = sum(Q_o < 0 & Q_u < 0, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  # Calculate percentages for each phase within each cycle
  mutate(
    ...
    water_stroomt_over_en_onder_stroomafwaarts = both_flow_pos / total_time_in_phase * 100,
    water_stroomt_over_en_onder_stroomopwaarts = both_flow_neg / total_time_in_phase * 100
  )  %>%
# 3. Calculate the average and median percentages across all tidal cycles
  ...
  # Voeg de rename() stap toe om de kolomnaam aan te passen
  rename(
    "Getij" = tide_direction
  ) %>% 
  kable(caption = "Percentage tijdsaandeel verschillende routes en richtingen water voor vloed en eb gedurende de studieperiode.")
```
...
Stroomafwaartse passage is het grootste deel van de tijd mogelijk (82.6 % van de tijd) over (60.8 % van de tijd) of onder de stuw (26.7 % van de tijd) (Fig. \@ref(fig:bartotaal)).
Indien de onderkant van de stuw open is (27.1 % van de tijd), dan is stroomafwaartse passage onder de stuw bijna altijd mogelijk (26.7 % van de tijd), maar stroomopwaartse passage onder de stuw veel minder (8.6 % van de tijd).
Dat komt omdat water veel vaker stroomafwaarts wordt weggelaten dan dat er stroomopwaarts wordt toegelaten (Tabel \@ref(tab:ebvloed)).
Er is maandelijkse variatie in deze verhoudingen, maar deze is niet zeer uitgesproken (Fig. \@ref(fig:barmaand)).

Elke dag zijn de mogelijkheden voor stroomafwaartse migratie aanzienlijk hoger dan voor stromopwaartse migratie (Fig. \@ref(fig:passeerbaargrafiekrichting)).

```{r bartotaal, fig.cap = "Barplot tijdsaandeel bereikbaarheid en passeerbaarheid vissen onder verschillende omstandigheden.", fig.width=12, fig.height=8, out.width='100%', fig.align='center'}
data %>%
  pivot_longer(
  ...
  geom_col() +
  # --- ADDED CODE: geom_text for bar labels ---
  geom_text(
    aes(label = scales::percent(aandeel_yes, accuracy = 0.1)),
    vjust = -0.5, # Positions the text just above the bar
    size = 4,     # Adjusts the text size
    color = "black" # Optional: ensures the text is visible
  ) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.1),
                     limits = c(0, 1)) + # Extends the y-axis to 110% to prevent labels from being clipped +
  labs(
...
```{r barmaand, fig.cap = "Barplot tijdsaandeel bereikbaarheid en passeerbaarheid vissen onder verschillende omstandigheden per maand.",fig.width=12, fig.height=20, out.width='100%', fig.align='center'}
data %>%
  ...
  geom_col() +
  geom_text(
    aes(label = scales::percent(aandeel_yes, accuracy = 1)),
    vjust = -0.5, # Positions the text just above the bar
    size = 4,     # Adjusts the text size
    color = "black" # Optional: ensures the text is visible
  ) +
  ...
  labs(
    title = "Aandeel momenten bereikbaar en passeerbaar per route × richting per maand",
    x = "Route × Richting", y = "Aandeel momenten",
    fill = "Categorie"
  ) +
  theme_minimal(base_size = 14) +
...
```{r passeerbaargrafiekrichting, fig.cap = "Daggemiddelde tijdsaandeel passeerbaarheid stroomopwaarts en stroomafwaarts.", fig.width=12, fig.height=8, out.width='100%', fig.align='center'}
data %>%
...
```{r passeerbaargrafiekrouterichting, fig.cap = "Daggemiddelde tijdsaandeel passeerbaarheid stroomopwaarts en stroomafwaarts, onder en over de stuw.", fig.width=12, fig.height=5, out.width='100%', fig.align='center'}
data %>%
  mutate(date = as.Date(date_utc)) %>%
  group_by(date) %>%
  summarise(
...
```{r passeerbaargrafiekrichtingroute, fig.cap = "Daggemiddelde tijdsaandeel passeerbaarheid onder en over de stuw, stroomopwaarts en stroomafwaarts.", fig.width=12, fig.height=5, out.width='100%', fig.align='center'}
data %>%
  mutate(date = as.Date(date_utc)) %>%
  group_by(date) %>%
  summarise(
...
Bij een debiet van 10 m³ en een opening onder de stuw van 0.5 m is vispassage mogelijk vanaf een waterhoogte boven de stuwkruin van 1 m (Fig. \@ref(fig:rangebovenstuw)).
...
Een datagedreven analyse m.b.v een beslissingsboom geeft aan hoe bij 'vrije stroming' een debiet lager dan 15 m³/s nodig is om vispassage over de stuw mogelijk te maken (Fig. \@ref(fig:dtfreeflowunder)).

```{r rangebovenstuw, fig.cap="Stroomsnelheid over de stuw ($V_o$ in m/s) in functie van de opening onder de stuw ($h_u$ in m) voor verschillende debieten.", fig.width=12, fig.height=16, out.width='100%', fig.align='center'}
...
Stroomopwaartse vispassage onder de stuw is enkel mogelijk indien het verschil tussen stroomopwaarts en stroomafwaarts peil minder dan 14 cm bedraagt (Fig. \@ref(fig:rangeonderstuw)).
Bij vrije stroming is in 76.3 % van de gevallen de stuw passeerbaar langs onderen.
Een datagedreven analyse m.b.v een beslissingsboom geeft aan hoe bij 'vrije stroming' een debiet lager dan 133 m³/s nodig is om vispassage onder de stuw mogelijk te maken (Fig. \@ref(fig:dtfreeflowover)).

```{r rangeonderstuw, fig.cap="Stroomsnelheid onder de stuw in functie van het hoogteverschil tussen het stroomopwaarts en stroomafwaarts waterpeil.", fig.width=8, fig.height=8, out.width='100%', fig.align='center'}
# Set the parameters for the plots
b <- 17
C_d <- 0.6
g <- 9.81
...
# Calculate the corresponding head difference (The value for the vertical line)
# Formula: delta_h = V_u^2 / (C_d^2 * 2 * g)
head_difference_target <- (V_u_target^2) / (C_d^2 * 2 * g)
# head_difference_target is approximately 0.1415788877562578

# Define a range for the head difference on the x-axis
# (h_up - h_down) can range from 0 up to a reasonable maximum
head_difference_range <- seq(from = 0, to = 4.0, length.out = 100)
...
ggplot(plot_data, aes(x = head_difference, y = V_u)) +
  ...
  # 1. Horizontal dotted line at V_u = 1 m/s
  geom_hline(yintercept = V_u_target, linetype = "dotted", color = "red", size = 0.8) +
  
  # 2. Vertical dotted line at the calculated head_difference
  geom_vline(xintercept = head_difference_target, linetype = "dotted", color = "red", size = 0.8) +
  
  # Optional: Mark the intersection point
  geom_point(aes(x = head_difference_target, y = V_u_target), color = "red", size = 3) +
  
  # 3. Label V_u on the horizontal line
  geom_text(
    aes(x = max(head_difference_range), y = V_u_target), 
    label = paste0("V_u = ", V_u_target, " m/s"), 
    hjust = 1.1, vjust = -0.5, color = "red", size = 3
  ) +
  
  # 4. Label head_difference on the vertical line
  geom_text(
    aes(x = head_difference_target, y = 0), 
    label = paste0("h_diff = ", round(head_difference_target, 4), " m"), 
    hjust = -0.1, vjust = 1.5, color = "red", size = 3
  ) +
  
  labs(
    x = "Hoogteverschil stroomopwaarts en stroomafwaarts waterpeil (h_up - h_down, in m)",
    y = "Stroomsnelheid onder stuw (V_u, in m/s)"
  ) +
  theme_minimal() 
```
...
## Gedetailleerde analyse van een aantal representatieve dagen

Op 15 januari 2024 was het business as usual met de bovenste schuif die naar omhoog wordt geschoven als het stroomafwaarts waterpeil het stroomopwaarts streefpeil van 5.77 m bereikt (Fig. \@ref(fig:tideplot20240115)).
...
```{r tideplot20240115, fig.width=12, fig.height=8, out.width='100%', fig.align='center', fig.cap="Tijdreeksen van waterpeil, stuwwerking en stroomsnelheid op 15 januari 2024. De gekleurde balken geven aan of vispassage over of onder de stuw mogelijk is. $h_upstream$ = stroomopwaarts waterpeil; $h_downstream$ = stroomafwaarts waterpeil; $h_o$ = waterhoogte boven stuwkruin; $h_si$ = hoogte bovenkant stuw; $sluice_opening$ = opening onder stuw; $V_o$ = stroomsnelheid over de stuw; $V_u$ = stroomsnelheid onder de stuw."}
...
```{r tideplot20240322, fig.width=12, fig.height=8, out.width='100%', fig.align='center', fig.cap="Tijdreeksen van waterpeil, stuwwerking en stroomsnelheid op 22 maart 2024. De gekleurde balken geven aan of vispassage over of onder de stuw mogelijk is. $h_upstream$ = stroomopwaarts waterpeil; $h_downstream$ = stroomafwaarts waterpeil; $h_o$ = waterhoogte boven stuwkruin; $h_si$ = hoogte bovenkant stuw; $sluice_opening$ = opening onder stuw; $V_o$ = stroomsnelheid over de stuw; $V_u$ = stroomsnelheid onder de stuw."}
...
```{r tideplot20240105, fig.width=12, fig.height=8, out.width='100%', fig.align='center', fig.cap="Tijdreeksen van waterpeil, stuwwerking en stroomsnelheid op 5 januari 2024. De gekleurde balken geven aan of vispassage over of onder de stuw mogelijk is. $h_upstream$ = stroomopwaarts waterpeil; $h_downstream$ = stroomafwaarts waterpeil; $h_o$ = waterhoogte boven stuwkruin; $h_si$ = hoogte bovenkant stuw; $sluice_opening$ = opening onder stuw; $V_o$ = stroomsnelheid over de stuw; $V_u$ = stroomsnelheid onder de stuw."}
...
```{r tideplot20240108, fig.width=12, fig.height=8, out.width='100%', fig.align='center', fig.cap="Tijdreeksen van waterpeil, stuwwerking en stroomsnelheid op 8 januari 2024. De gekleurde balken geven aan of vispassage over of onder de stuw mogelijk is. $h_upstream$ = stroomopwaarts waterpeil; $h_downstream$ = stroomafwaarts waterpeil; $h_o$ = waterhoogte boven stuwkruin; $h_si$ = hoogte bovenkant stuw; $sluice_opening$ = opening onder stuw; $V_o$ = stroomsnelheid over de stuw; $V_u$ = stroomsnelheid onder de stuw."}
plot_tidal_cycle_analysis_combined(data, chosen_date = as.POSIXct("2024-01-08", tz = "UTC"))
```

```{r tideplot2024, fig.width=12, fig.height=8, out.width='100%', fig.align='center', fig.cap="Tijdreeksen van waterpeil, stuwwerking en stroomsnelheid van 1 januari 2024 tot en met 31 maart 2024. $h_upstream$ = stroomopwaarts waterpeil; $h_downstream$ = stroomafwaarts waterpeil; $h_o$ = waterhoogte boven stuwkruin; $h_si$ = hoogte bovenkant stuw; $sluice_opening$ = opening onder stuw; $V_o$ = stroomsnelheid over de stuw; $V_u$ = stroomsnelheid onder de stuw."}
output="pdf"
if (output=="html"){ggplotly(plot_tidal_cycle_analysis_combined(data, chosen_date = "full"))}
```

## Stuwwerking van stuw 222 te Mechelen

**Stuw 222 werd bezocht op 1 september 2025 en is sinds 25 juni 2025 weer in werking, na twee jaar buiten bedrijf te zijn geweest, een periode waarin het getij vrij spel had.** Gedurende die twee jaar waren er geen noemenswaardige structurele problemen, maar de vrije getijwerking bemoeilijkte wel de regeling van het streefpeil van de Binnendijle (4,25 m TAW) door leegloop bij eb.
Daarnaast meldde men grotere druk op de oevers door periodieke schommelingen in de waterstand en waren er aanwijzingen voor een grotere droogtestress in stroomopwaartse natte natuurgebieden.
**Sinds de heropstart komt water enkel stroomopwaarts over de stuw bij hoogwater, met een nagestreefd streefpeil van 4,8 m TAW.** Echter, volgens Waterinfo lijkt het erop dat eerder een peil van **4,5 m TAW** wordt nagestreefd op dit moment.
Om de werking van de stuw te beoordelen zijn logging van de stuwwerking vereist maar die ontbreken momenteel nog.
